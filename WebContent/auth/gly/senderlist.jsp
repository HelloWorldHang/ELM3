...
<a href="${pageContext.request.contextPath }/auth/gly/addsender.jsp"><input type="submit" value="添加骑手"/></a>

	<form action="${pageContext.request.contextPath }/servlet/sender" method="post">
	...
	<table width="100%" cellspacing="0">
		...
		<c:forEach items="${requestScope.senderList }" var="sender">
			<tr>
				<td>${sender.id }</td>
				<td>${sender.username }</td>
				<td>${sender.tel }</td>
				<td>
					<img alt="sender" width="80px" height="80px" src="${pageContext.request.contextPath }${sender.img }">
				</td>
				<td>${sender.idcard }</td>
				<td>
					<a href="${pageContext.request.contextPath }/servlet/sender?op=findById&id=${sender.id}&&who=gly">修改</a>
				</td>
				<td>
					<a href="${pageContext.request.contextPath }/servlet/sender?op=delete&id=${sender.id}">删除</a>
				</td>
			</tr>
		</c:forEach>
	</table>
